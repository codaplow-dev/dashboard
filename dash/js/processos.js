const processos = [
    { id: 1, numero: "2026/001245", assunto: "Solicitação de licença", status: "Em andamento", setor: "GRH", responsavel: "Ana Silva", prioridade: "Alta", data: "19/08/2026", descricao: "Solicitação de licença funcional." },
    { id: 2, numero: "2026/001198", assunto: "Alteração cadastral", status: "Concluído", setor: "SEFA", responsavel: "Carlos Souza", prioridade: "Baixa", data: "18/08/2026", descricao: "Atualização dos dados cadastrais do servidor." },
    { id: 3, numero: "2026/001176", assunto: "Solicitação de acesso", status: "Pendente", setor: "TI", responsavel: "Marcos Lima", prioridade: "Alta", data: "17/08/2026", descricao: "Aguardando aprovação para liberação de acesso ao sistema." },
    { id: 4, numero: "2026/001155", assunto: "Criação de usuário", status: "Em andamento", setor: "TI", responsavel: "Pedro Henrique", prioridade: "Média", data: "16/08/2026", descricao: "Criação de credencial de acesso para novo colaborador." },
    { id: 5, numero: "2026/001102", assunto: "Análise documental", status: "Concluído", setor: "Protocolo", responsavel: "Juliana Costa", prioridade: "Baixa", data: "15/08/2026", descricao: "Documentação analisada e encaminhada ao setor responsável." },
    { id: 6, numero: "2026/001087", assunto: "Homologação de solicitação", status: "Pendente", setor: "GRH", responsavel: "Fernanda Alves", prioridade: "Média", data: "14/08/2026", descricao: "Processo aguardando homologação da chefia." },
    { id: 7, numero: "2026/001054", assunto: "Solicitação de parecer", status: "Em andamento", setor: "Jurídico", responsavel: "Ricardo Martins", prioridade: "Alta", data: "13/08/2026", descricao: "Solicitação de parecer jurídico para continuidade do processo." }
];
...
function statusClass(status) {
    return status === "Em andamento" ? "andamento" : status === "Concluído" ? "concluido" : "pendente";
}
...
function renderProcesses(list = processos) {
    ...
    processList.innerHTML = list.map(process => `
...
function filterProcesses() {
    ...
    const result = processos.filter(process => {
        const matchesSearch = [process.numero, process.assunto, process.setor, process.responsavel].some(value => value.toLowerCase().includes(term));
...
function openModal(content) {
    ...
    document.body.insertAdjacentHTML("beforeend", `<div class="modal-overlay active"><div class="process-modal" role="dialog" aria-modal="true">${content}</div></div>`);
...
function openNewProcessForm() {
    ...
    document.querySelectorAll(".close-modal").forEach(button => button.addEventListener("click", () => document.querySelector(".modal-overlay")?.remove()));
    document.querySelector("#newProcessForm").addEventListener("submit", event => {
        event.preventDefault();
        const getValue = id => document.querySelector(`#${id}`).value.trim();
        processos.unshift({ id: Date.now(), numero: getValue("numero"), assunto: getValue("assunto"), setor: getValue("setor"), responsavel: getValue("responsavel"), status: getValue("status"), prioridade: getValue("prioridade"), descricao: getValue("descricao") || "Sem descrição.", data: new Intl.DateTimeFormat("pt-BR").format(new Date()) });
        document.querySelector(".modal-overlay")?.remove();
        filterProcesses();
    });
}
